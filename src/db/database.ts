...
export async function addCorpus(item: {
  title: string;
  content: string;
  type: 'markdown' | 'text';
}): Promise<void> {
  try {
    const database = await getDB();
    await database.executeSql(
      'INSERT INTO corpus (title, content, type) VALUES (?, ?, ?)',
      [item.title, item.content, item.type],
    );
  } catch (e) {
    console.warn('[DB] add corpus failed:', e);
  }
}

export async function loadCorpus(): Promise<CorpusRow[]> {
  try {
    const database = await getDB();
    const [results] = await database.executeSql(
      'SELECT id, title, content, type, enabled, created_at FROM corpus ORDER BY id DESC',
    );
    const rows: CorpusRow[] = [];
    for (let i = 0; i < results.rows.length; i++) {
      rows.push(results.rows.item(i) as CorpusRow);
    }
    return rows;
  } catch (e) {
    console.warn('[DB] load corpus failed:', e);
    return [];
  }
}
...
export async function updateCorpus(
  id: number,
  updates: {title?: string; content?: string; enabled?: number},
): Promise<void> {
  try {
    const database = await getDB();
    ...
    if (setClauses.length > 0) {
      params.push(id);
      await database.executeSql(
        `UPDATE corpus SET ${setClauses.join(', ')} WHERE id = ?`,
        params,
      );
    }
  } catch (e) {
    console.warn('[DB] update corpus failed:', e);
  }
}
...
export async function deleteCorpus(id: number): Promise<void> {
  try {
    const database = await getDB();
    await database.executeSql('DELETE FROM corpus WHERE id = ?', [id]);
  } catch (e) {
    console.warn('[DB] delete corpus failed:', e);
  }
}

export async function loadEnabledCorpusContent(): Promise<string> {
  try {
    const database = await getDB();
    const [results] = await database.executeSql(
      'SELECT title, content, type FROM corpus WHERE enabled = 1 ORDER BY id ASC',
    );
    let combined = '';
    for (let i = 0; i < results.rows.length; i++) {
    ...
    console.warn('[DB] load enabled corpus failed:', e);
    return '';
  }
}
